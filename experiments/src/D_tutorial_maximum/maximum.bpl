...
// Command Line Options: /compile:0 /noVerify /print:maximum.bpl maximum.dfy
...
implementation CheckWellformed$$_module.__default.Maximum(values#0: Seq Box) returns (max#0: int)
{
  ...
  var i#0: int;
    ...
    assume {:captureState "maximum.dfy(8,7): initial state"} true;
    ...
    havoc max#0;
    assume {:captureState "maximum.dfy(10,14): post-state"} true;
...
procedure Call$$_module.__default.Maximum(values#0: Seq Box
  ...
  free ensures (forall i#1: int :: { $Unbox(Seq#Index(values#0, i#1)): int } true);
...
implementation Impl$$_module.__default.Maximum(values#0: Seq Box) returns (max#0: int, $_reverifyPost: bool)
{
  ...
  var $decr$loop#00: int;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha ::
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "maximum.dfy(12,0): initial state"} true;
    $_reverifyPost := false;
    // ----- assignment statement ----- maximum.dfy(13,7)
    assume true;
    assert 0 <= LitInt(0) && LitInt(0) < Seq#Length(values#0);
    assume true;
    max#0 := $Unbox(Seq#Index(values#0, LitInt(0))): int;
    assume {:captureState "maximum.dfy(13,18)"} true;
    // ----- assignment statement ----- maximum.dfy(14,11)
    assume true;
    assume true;
    idx#0 := LitInt(0);
    assume {:captureState "maximum.dfy(14,14)"} true;
    // ----- while statement ----- maximum.dfy(15,3)
    ...
    while (true)
      ...
      free invariant $w$loop#0
         ==> (forall j#1: int :: { $Unbox(Seq#Index(values#0, j#1)): int } true);
      invariant $w$loop#0
         ==> (forall j#1: int ::
    ...
    {
        assume {:captureState "maximum.dfy(15,2): after some loop iterations"} true;
        if (!$w$loop#0)
        {
            assume true;
            assume Seq#Contains(values#0, $Box(max#0));
            ...
            assume (forall j#1: int :: { $Unbox(Seq#Index(values#0, j#1)): int } true);
        ...
        // ----- if statement ----- maximum.dfy(20,5)
        assert 0 <= idx#0 && idx#0 < Seq#Length(values#0);
        assume true;
        if ($Unbox(Seq#Index(values#0, idx#0)): int > max#0)
        {
            // ----- assignment statement ----- maximum.dfy(21,11)
            assume true;
            assert 0 <= idx#0 && idx#0 < Seq#Length(values#0);
            assume true;
            max#0 := $Unbox(Seq#Index(values#0, idx#0)): int;
            assume {:captureState "maximum.dfy(21,24)"} true;
        }
        else
        {
        }

        // ----- assignment statement ----- maximum.dfy(23,9)
        assume true;
        assume true;
        idx#0 := idx#0 + 1;
        assume {:captureState "maximum.dfy(23,18)"} true;
        // ----- loop termination check ----- maximum.dfy(15,3)
...
implementation CheckWellformed$$_module.__default.MaximumIsUnique(values#0: Seq Box, m1#0: int, m2#0: int)
{
  ...
  var i#2: int;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha ::
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "maximum.dfy(27,6): initial state"} true;
    ...
    assume {:captureState "maximum.dfy(30,13): post-state"} true;
    assume m1#0 == m2#0;
}
...
implementation Impl$$_module.__default.MaximumIsUnique(values#0: Seq Box, m1#0: int, m2#0: int) returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha ::
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "maximum.dfy(30,19): initial state"} true;
    $_reverifyPost := false;
}
