...
instance Pretty Program where 
  pretty (Program decls _comments) = (vsep . punctuate linebreak . map pretty) decls
...
instance Pretty Type where 
  ...
  pretty (MapType fv domains range) = typeArgsDoc fv <> 
    ...
    pretty range
  pretty (IdType id []) = text id
  -- pretty (IdType id [x]) = text id <+> pretty x 
  pretty (IdType id [x]) = text id <+> argsDoc
    where argsDoc = case x of
            IdType _ _ -> parens (pretty x)
            otherwise  -> (pretty x)
