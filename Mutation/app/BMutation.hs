...
xrayLoop numberOfMutants tranPool mutantPool = do
  ...
  let mu = tranPool !! index
      mo = mutantPool !! (index `mod` (length mutantPool))
...
mainWithConfig = do
  ...
  muts <- generateMutations sourceProg (1000) (mutationRatio config)
  let origin = Mutant [sourceProg] (sourceBoogie config) sourceProg [] 
  mutants <- xrayLoop (numberOfMutants config) muts [origin]
  return ()
